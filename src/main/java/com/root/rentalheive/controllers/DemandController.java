...
//        @Query("SELECT r FROM Rental r WHERE r.equipment = ?1 AND (r.rentalDate >= ?2 AND r.rentalDate <= ?3) OR (r.rentalDate <= ?2 AND r.returnDate >= ?2)")
//        List<Rental> findAllCurrentRentalsByEquipment(Equipment equipment, LocalDate startDate, LocalDate endDate);
